...
public class PossibleStringCount {
    ...
    public static boolean canPartitionKSubsets(int[] nums, int k) {
        ...
        if (nums[n - 1] > target) return false; // 最大值比目标和还大，肯定不行

        int[] bucket = new int[k];
        return backtrack(nums, n - 1, bucket, target);
    }
    ...
    public static boolean canPartitionKSubsetsDp1(int[] nums, int k) {
        int sum = 0;
        for (int num : nums) {
            sum += num;
        }
    ...
    public static boolean canPartitionKSubsetsDp2(int[] nums, int k) {
        int sum = 0;
        for (int num : nums) {
            sum += num;
        }
