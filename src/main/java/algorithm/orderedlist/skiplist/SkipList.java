package algorithm.orderedlist.skiplist;
// 跳表的实现(java版)
...
public class SkipList {
    ...
    public static void removeNode(int i, int h, int j) {
        ...
        while (next[i][j] != 0 && key[next[i][j]] < key[j]) {
            i = next[i][h];
        }
        if (h > level[j]) {
            len[i][h]--;
    ...
    public static int pre(int i, int h, int num) {
        ...
        if (h == 1) {
            return i == 1 ? Integer.MAX_VALUE : key[i];
        } else {
